$(function(){
    ...
    var btn_enviar_producto = $("#guardar_business_user");
    $("#guardar_business_user").on('click', function() {
        inputs = [];
        msgError = '';
        ...
        validarSelect($('select#business_id')) == false ? inputs.push('Empresa') : ''


        if (inputs.length == 0) {
            $('#guardar_business_user').hide();
            saveBusinessAdmin();
        } else {
            $('#guardar_business_user').show();
    ...
    function validarInput (campo,regExpr) {
        if($('form#form_business_user input#id').val() != '' && $(campo).attr('name') == 'password' && $(campo).val() == '') {
            return true;
        }
